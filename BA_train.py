...
train_data_path = "data/syn/dataset_{}_{}_{}nodes_train_{}.pickle".format(graph_type, edge_type, N, scale)
test_data_path = "data/syn/dataset_{}_{}_{}nodes_test_{}.pickle".format(graph_type, edge_type, N, scale)
val_data_path = "data/syn/dataset_{}_{}_{}nodes_val_{}.pickle".format(graph_type, edge_type, N, scale)
...
#             lossgmse = get_frobenius_norm_except_diag(theta_pred - theta_true[ridx])
        ...
        if len(valid_data) > 0:
            ...
            curr_valid_ps, curr_valid_aps, curr_valid_aucs, curr_valid_nmse, \
            curr_valid_gmse, curr_valid_gmse_ediag = glasso_predict(model, valid_data)
            curr_train_ps, curr_train_aps, curr_train_aucs, curr_train_nmse, \
            curr_train_gmse, curr_train_gmse_ediag = glasso_predict(model, data)
            print('valid/train: ps %0.2f/%0.2f aps %0.2f/%0.2f aucs %0.2f/%0.2f nmse %0.2f/%0.2f' % (
            curr_valid_ps, curr_train_ps, curr_valid_aps, curr_train_aps,
            curr_valid_aucs, curr_train_aucs, curr_valid_nmse, curr_train_nmse))
...
def glasso_predict(model, data, flagP=False, SAVE_GRAPH=False, eM=0, name='', mn=''):
    with torch.no_grad():
        ...
        for batch_num in range(num_batches):  # processing batchwise
            ...
            FDRs, TPRs, FPRs, SHDs, Ts, Ps, precisions, recalls, F_betas, auprs, aucs = metrics.report_metrics(theta_true_b, theta_pred)
...
#                 print(i, *sum(list(map(list, zip(mean_vec, std_vec))), []), sep=', ')
        ...
        return np.float(res_mean[5]), np.float(res_mean[9]), np.float(res_mean[10]), final_nmse, final_gmse, final_gmse_exp_diag  # The PS mean value, final NMSE obtained
...
if TRAIN == True:
    ...
    nmse_model, shd_model, ps_model = train_glasso(train_data, val_data)
