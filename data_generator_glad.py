...
class create_MN_vary_w(object):
    ...
    def init_random_graph(self, graph_type, seed=None, u=1):  # init random adjacency matrix with probability as p
        ...
        cov = np.linalg.inv(
            precision_mat)  # avoiding the use of pinv as data not true representative of conditional independencies.
...
def convert_to_torch(data, TESTING_FLAG=False, USE_CUDA=False):# convert from numpy to torch variable
    if USE_CUDA == False:
